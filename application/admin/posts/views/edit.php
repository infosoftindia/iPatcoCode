						...
						<?php
							...
							if($post_Cats){
								foreach($post_Cats as $category){
									$cats[] = $category['post_category_category'];
								}
							}
						?>
						<?php
											...
											<div class="card">
												...
												<div class="card-body">
													<div class="form-group mb-3">
														...
														<input type="text" class="form-control" id="name" placeholder="Post Title" value="<?=set_value('title') != ''?set_value('title'):$post['posts_title']?>" name="title">
													</div>
													<div class="form-group  mb-3">
														<label for="short_desc">Excert Description</label>
														<textarea class="form-control" id="short_desc" rows="2" placeholder="Excert Description" name="short_desc"><?=set_value('short_desc') != ''?set_value('short_desc'):$post['blogs_excert']?></textarea>
													</div>
													<div class="form-group  mb-3">
														<label for="desc">Description</label>
														<textarea class="form-control editor" id="desc" rows="6" placeholder="Product Description" name="description"><?=set_value('description') != ''?set_value('description'):$post['blogs_description']?></textarea>
													</div>
												</div>
											</div>
										</div>
													...
													<div class="form-group">
														<select class="form-control selectF" name="category[]" multiple style="width: 100%">
															<?php if($categories){ foreach($categories as $category){ ?>
																...
																<?=(in_array($category->categories_id, $cats))?'selected':''?>
													...
													<div class="form-group">
														<select class="form-control selectF" name="tags[]" multiple style="width: 100%">
															<?php if($tags){ foreach($tags as $tag){ ?>
																<option value="<?=$tag->tags_id?>" <?=(in_array($tag->tags_id, $tagss))?'selected':''?>><?=$tag->tags_name?></option>
															<?php } } ?>
														</select>
													</div>
												</div>
												...
												<div class="card-body">
													<div class="form-group">
														...
														<select class="form-control form-control-sm" id="status" name="status">
															<option value="1" <?=$post['posts_status'] == '1'?'selected':''?>>Active</option>
															<option value="0" <?=$post['posts_status'] == '0'?'selected':''?>>Inactive</option>
														</select>
													</div>
												</div>
											</div>
